...
function RetryConnection(props) {
    
    const onRetry = () => {
        props.setShowRetryGetConfig(false);
        props.setRetryConfig(true);
        props.setShowInsertDl(true);
    };

    const onCancel = () => {
        props.setRetryConfig(false);
        props.setShowRetryGetConfig(false);
    }
    ...
    return (
        <div style={{ minWidth:'50%',position: 'absolute', left:'50%',top:'50%',
            ...
            <Paper elevation={3} 
                ...
                <br/>
                <Button variant="contained" style={{position: 'relative', left: '25%'}}
                onClick={()=>onRetry()}>
                    Retry
                </Button>
                <Button variant="contained" style={{position: 'relative', left: '-25%'}}
